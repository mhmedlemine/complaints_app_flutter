...
import 'package:complaintsapp/presentation/complaints/add_consumer_screen.dart';
...
class ConsumerSearchScreen extends StatefulWidget {
  ...
  @override
  State<ConsumerSearchScreen> createState() => _ConsumerSearchScreenState();
}
...
class _ConsumerSearchScreenState extends State<ConsumerSearchScreen> {
              ...
              child: TextFormField(
                ...
                decoration: InputDecoration(
                  hintText: widget.hintTxt ??
                      'complaint_enter_consumer_phone_number',
                  errorText: null,
                  counterText: '',
          ...
          if (_queryTextController.text.length == 8) ... [
            ListTile(
              onTap: () async {
                ...
                final res = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => AddConsumerScreen(phoneNumber: phoneNumber)));
                if (res != null) {
              ...
              subtitle: Text('complaint_consumer_does_not_exist', style: TextStyle(color: Colors.red),),
  ...
  Future<void> doSearch(String value) async {
    if (mounted && value.length > 1) {
      ...
      final consumers = await _checkupStore.searchConsumers(value);

      final response = AutocompleteResponse(
        error: _checkupStore.errorStore.errorMessage,
        errorDescription: _checkupStore.errorStore.errorMessage,
        suggestions: consumers.consumers ?? [],
      );
...
class AutocompleteResponse {
  ...
  final List<Consumer> suggestions;
